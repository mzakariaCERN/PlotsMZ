...
void StudyFiles(std::vector<TString> file_names, int foi, hist_class *my_hists, bool is_pp)
{
  ...
  //-------------------------------------------------------------
  int pt_weight_bin;
  double pbpb_pt, pp_pt, pt_weight_inc, pt_weight_sub, pt_weight_lead; 
  ...
  for(int ibin = 0; ibin<4; ibin++){
    ...
    pbpb_spectrum_inc[ibin]->SetName((TString)("PbPb_jet_spectrum_inc_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));

    pbpb_spectrum_lead[ibin] = (TH1D*)f_ref_pbpb_spectra->Get((TString)("only_leadingjets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
    pbpb_spectrum_lead[ibin]->SetName((TString)("PbPb_jet_spectrum_lead_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));

    pbpb_spectrum_sub[ibin] = (TH1D*)f_ref_pbpb_spectra->Get((TString)("only_subleadingjets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
    pbpb_spectrum_sub[ibin]->SetName((TString)("PbPb_jet_spectrum_sub_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));


    pp_spectrum_inc[ibin] = (TH1D*)f_ref_pp_spectra->Get((TString)("all_jets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
    pp_spectrum_inc[ibin]->SetName((TString)("pp_jet_spectrum_inc_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));

    pp_spectrum_lead[ibin] = (TH1D*)f_ref_pp_spectra->Get((TString)("only_leadingjets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
    pp_spectrum_lead[ibin]->SetName((TString)("pp_jet_spectrum_lead_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));

    pp_spectrum_sub[ibin] = (TH1D*)f_ref_pp_spectra->Get((TString)("only_subleadingjets_corrpT_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+"_Pt100_Pt300"));
    pp_spectrum_sub[ibin]->SetName((TString)("pp_jet_spectrum_sub_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]));
  ...
  for(int fi = 0; fi < (int) file_names.size(); fi++) {
	...
	if(!is_inclusive&&j4i!=highest_idx&&j4i!=second_highest_idx){continue; }
	...
	for (int ibin=0;ibin<nCBins; ibin ++){
	  ...
	  pt_weight_inc = pt_weight_lead = pt_weight_sub = 1.;
	  ...
	  if(is_pp){
	    ...
	    pp_pt = pp_spectrum_inc[ibin]->GetBinContent(pt_weight_bin);  /// pp data
	    pt_weight_inc = 1.;
	    if(pp_pt>0.0001){
	      pt_weight_inc = pbpb_pt / pp_pt;
	    }
	  
	    pt_weight_bin = pbpb_spectrum_lead[ibin]->GetXaxis()->FindBin(my_primary->jtpt->at(j4i));
	    pbpb_pt = pbpb_spectrum_lead[ibin]->GetBinContent(pt_weight_bin);  /// PbPb data
	    pp_pt = pp_spectrum_lead[ibin]->GetBinContent(pt_weight_bin);  /// pp data
	    pt_weight_lead = 1.;
	    if(pp_pt>0.0001){ 
	      pt_weight_lead = pbpb_pt / pp_pt;
	    }


	    pt_weight_bin = pbpb_spectrum_sub[ibin]->GetXaxis()->FindBin(my_primary->jtpt->at(j4i));
	    pbpb_pt = pbpb_spectrum_sub[ibin]->GetBinContent(pt_weight_bin);  /// PbPb data
	    pp_pt = pp_spectrum_sub[ibin]->GetBinContent(pt_weight_bin);  /// pp data
	    pt_weight_sub = 1.;
	    if(pp_pt>0.0001){ 
	      pt_weight_sub = pbpb_pt / pp_pt;
	    }
	  
	    //  cout<<my_primary->jtpt->at(j4i)<<" "<<pt_weight_bin<<" "<<pt_weight_inc<<" "<<pt_weight_lead<<" "<<pt_weight_sub<<endl;
	  }
	  //-----------------------------------
	  ...
	  for(int tracks =0; tracks < (int) my_primary->trkPt->size(); tracks++){
	    ...
	    trkweight = (1-fake)*(1-secondary)/eff;
	    ...
	    my_hists->TrkPt_weighted[ibin][ibin2][ibin3]->Fill(my_primary->trkPt->at(tracks),trkweight*pt_weight_inc*wvz*wcen);
	    my_hists->TrkEta_weighted[ibin][ibin2][ibin3]->Fill(my_primary->trkEta->at(tracks),trkweight*pt_weight_inc*wvz*wcen);
	    my_hists->TrkPhi_weighted[ibin][ibin2][ibin3]->Fill(my_primary->trkPhi->at(tracks),trkweight*pt_weight_inc*wvz*wcen);
	    ...
	    if(is_inclusive == kTRUE){
	      ...
	      my_hists->hJetTrackSignalBackground[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_inc*wvz*wcen);
	    ...
	    if(j4i==highest_idx){
	      ...
	      my_hists->hJetTrackSignalBackgroundLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_lead*wvz*wcen);
	    ...
	    if(j4i==second_highest_idx){
	      ...
	      my_hists->hJetTrackSignalBackgroundSubLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_sub*wvz*wcen);
	    ...
	    if(is_inclusive && j4i!=highest_idx){
	      ...
	      my_hists->hJetTrackSignalBackgroundNonLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_inc*wvz*wcen);
	...
	if(is_inclusive||j4i==highest_idx||j4i==second_highest_idx){  //only if we've got a trigger according to some criteria  AND we're not pp
	  ...
	  while(mevi< meptrig){  //
		...
		my_hists->ME_TrkPt_weighted[ibin][ibin2][ibin3]->Fill(me_tree->trkPt->at(tracks),trkweight*pt_weight_inc*wvz*wcen);
		my_hists->ME_TrkEta_weighted[ibin][ibin2][ibin3]->Fill(me_tree->trkEta->at(tracks),trkweight*pt_weight_inc*wvz*wcen);
		my_hists->ME_TrkPhi_weighted[ibin][ibin2][ibin3]->Fill(me_tree->trkPhi->at(tracks),trkweight*pt_weight_inc*wvz*wcen);
		...
		if(is_inclusive){
		  ...
		  my_hists->hJetTrackME[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_inc*wvz*wcen);
		...
		if(j4i==highest_idx){
		  ...
		  my_hists->hJetTrackMELeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_lead*wvz*wcen);
		...
		if(j4i==second_highest_idx){
		  ...
		  my_hists->hJetTrackMESubLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_sub*wvz*wcen);
		...
		if(is_inclusive && j4i!=highest_idx){
		  ...
		  my_hists->hJetTrackMENonLeading[ibin][ibin2][ibin3]->Fill(deta,dphi, trkweight*pt_weight_inc*wvz*wcen);
