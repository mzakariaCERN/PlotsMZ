	...
	int Study_LevelTwo_NoAjCut_Leading_OriginalJEC_20150431(double i=0, double j = 0)
	{
	...
	TFile *fin = TFile::Open("/home/mzakaria3/Documents/Research/MyGitProject/PlotsMZ/Data2011_NoAjCut_Only13_20150511.root");  // The input file
	TFile *fin_LevelOne = TFile::Open("~/Documents/Research/MyGitProject/PlotsMZ/BackUpCode_Analyze_NoAjCut_Data2011_Only13_20150431_OriginalJEC/LevelOne/LevelOne_PbPb_NoAjCut22_Correlations_20150431_OriginalJEC.root");  // The input file
	...
	fout_inc = new TFile("LevelTwo_PbPb_NoAjCut_Correlations_20150431.root","RECREATE"); // The output file
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...

	{
					...
					pttex->SetNDC();
					pttex->Draw();
					TLatex *Ajtex = new TLatex(0.75,0.85,AjBin_labels[1]);
					Ajtex->SetNDC();
					Ajtex->Draw();
			...
			dd1->SaveAs("dd1_PhiProjectionofSignalandBands_Leading_NoAjCut.png");	
	...
	//		Signal[ibin][ibin2][ibin3]->GetXaxis()->SetRangeUser(-1.5, 1.5);
			...
			dd2->SaveAs("dd2_resultOnPhi_Leading_NoAjCut.png");
...
//			check_old_phi_rebin[ibin][ibin2][ibin3]->SetMaximum(TMath::Max(check_old_phi_rebin[ibin][ibin2][ibin3]->GetMaximum()+5.0, check_old_phi_rebin[ibin][ibin2][ibin3]->GetMaximum())*1.05);
			...
			//check_new_phi[ibin][ibin2][ibin3]->Draw();
				...
				if(ibin==3){
					...
					pttex2->SetNDC();
					pttex2->Draw();
					TLatex *Ajtex2 = new TLatex(0.85,0.90,AjBin_labels[1]);
					Ajtex2->SetNDC();
					Ajtex2->Draw();
				...
				//	pttex->SetNDC();
				//	pttex->Draw();
				//	TLatex *Ajtex = new TLatex(0.80,0.90,AjBin_labels[1]);
				//	Ajtex->SetNDC();
				//	Ajtex2->Draw();
			...
			dd3->SaveAs("dd3_resultOnPhi_Leading_NoAjCut.png");
...
//		std::cout<<"AAA Bin Content:"<<Signal[ibin][ibin2][ibin3]->GetBinContent(1)<<std::endl;	
	...
	Signal[ibin][ibin2][ibin3]->Draw("same");
					...
					pttex->SetNDC();
					pttex->Draw();
					TLatex *Ajtex_dd3 = new TLatex(0.75,0.85,AjBin_labels[1]);
					Ajtex_dd3->SetNDC();
					Ajtex_dd3->Draw();
...
//			SignalMinusRL[ibin][ibin2][ibin3]->SetMaximum(TMath::Max(SignalMinusRL[ibin][ibin2][ibin3]->GetMaximum()+5.0, SignalMinusRL[ibin][ibin2][ibin3]->GetMaximum()*1.05));
				...
				
			{
					...
					pttex->SetNDC();
					pttex->Draw();
					TLatex *Ajtex_cc10 = new TLatex(0.75,0.85,AjBin_labels[1]);
					Ajtex_cc10->SetNDC();
					Ajtex_cc10->Draw();
			...
			cc10->SaveAs("cc10_resultOnPhi_Leading_NoAjCut.png");
		...
		//	SignalMinusRL[ibin][ibin2][ibin3]->Scale(1/(3.0 *  Aj[ibin]->GetEntries() * PhiBinWidth));
				...
				
			{
					...
					pttex->SetNDC();
					pttex->Draw();
					TLatex *Ajtex = new TLatex(0.85,0.85,AjBin_labels[1]);
					Ajtex->SetNDC();
					Ajtex->Draw();
		...
		cc11->SaveAs("cc11_resultOnPhi_Leading_NoAjCut.png");	
	//##///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				hJetTrackMELeading[ibin][ibin2][ibin3]->Draw("LEGO2");
	{
			...
			//		return 1;	
				...
				//if(ibin<3)
				{
					...
					pttex->SetNDC();
					pttex->Draw();
					TLatex *Ajtex = new TLatex(0.76,0.85,AjBin_labels[1]);
					Ajtex->SetNDC();
					Ajtex->Draw();
...
//				cc17->SaveAs("cc17_MEOverMEO_Leading_Under22.png");
...

	{
      			...
      			cc18->SaveAs("cc18_MEOverMEO_JetScaled_perEtaperPhi_Leading_NoAjCut.png");
...
//			hJetTrackSignalBackgroundLeading[ibin][ibin2][ibin3]->Scale(1.0 / ( Aj[ibin]->GetEntries() * PhiBinWidth*EtaBinWidth));	
	...
	//				if(ibin<3){
					...
					pttex23->SetNDC();
					pttex23->Draw();
					TLatex *Ajtex23 = new TLatex(0.80,0.90,AjBin_labels[1]);
					Ajtex23->SetNDC();
					Ajtex23->Draw();
...
//				background[ibin][ibin2][ibin3]->Draw("LEGO2");		
	...
	//			}
				...
				cc24->SaveAs("CC24_S_Scaled_overMEoverM0_NotScaled_PerEtaPerPhi_ProjectionY_etaSideBand_NoAjCut.png");	
...
//					Jettex->SetNDC();
	...
	//			}
				...
				cc25->SaveAs("CC25_S_Scaled_overMEoverM0_NotScaled_PerEtaPerPhi_ProjectionY_etaSideBand_NoAjCut.png");	
...
//return 26;
	...
	//	result[ibin][ibin2][ibin3]->Draw("LEGO2");
				...
				if(ibin==3){
					...
					pttex2->SetNDC();
					pttex2->Draw();
					TLatex *Ajtex2 = new TLatex(0.80,0.90,AjBin_labels[1]);
					Ajtex2->SetNDC();
					Ajtex2->Draw();
				...
				cc26->SaveAs("CC26_S_Scaled_overMEoverM0_NotScaled_PerEtaPerPhi_MinusEtaSideBand_ProjectionY_NoAjCut.png");	
...
//				background_proj[ibin][ibin2][ibin3]->Draw();
		...
		//From H	check_old_phi[g][i][j][l] = result2[g][i][j][l]->ProjectionY(summedchecknamePhi,llimiteta,rlimiteta);
			...
			//	check_old_phi_rebin[ibin][ibin2][ibin3]->Draw("Y-", "SAME");
				...
				cc27->SaveAs("CC27_S_ScaledMEoverM0_NotScaled_PerEtaPerPhi_MinusEtaSideBand_ProjectionY_NoAjCut.png");
	...
	//			Signal[ibin][ibin2][ibin3] -> Scale(1. / (rlimiteta - llimiteta+1))	;
		...
		//		Signal[ibin][ibin2][ibin3] -> Draw();
			...
			//	Signal[ibin][ibin2][ibin3]->SetMaximum(MaxSignal[ibin][ibin3]+30);
				...
				cc28->SaveAs("CC28_SubtractBeforeFilling_NoAjCut.png");
