...
//	TCanvas *c2 = new TCanvas("c2", "C2_SignalOverME_projPhi_LB", 0, 0, 1500, 1500);	
	...
	TCanvas *cc3 = new TCanvas("cc3", "CC3_JetTrackOverNormalizedME", 0, 0, 1500, 1500);	
	TCanvas *cc4 = new TCanvas("cc4", "CC4_JetTrackOverNormalizedME_etaProj", 0, 0, 1500, 1500);	
	TCanvas *cc5 = new TCanvas("cc5", "CC5_JetTrackOverNormalizedME_etaProj_LB_&_RB", 0, 0, 1500, 1500);	
...
//	c1->Divide(4,4,0.0,0.0);
	...
	cc3->Divide(4,4,0.0,0.0);
	cc4->Divide(4,4,0.0.0001,0.0001);
	cc5->Divide(4,4,0.0,0.0);
	cc6->Divide(4,4,0.0,0.0);
	cc7->Divide(4,4,0.0,0.0);
	cc8->Divide(4,4,0.0,0.0);

	

	TF1 *fa = new TF1("fa","[0]",-5,5);
	TF1 *flat = new TF1("flat_fit", "[0] + x - x " );	
		...
		//		printf("the line number is: %d \n \n", __LINE__);
			...
			//	hJetTrackSignalBackground[ibin][ibin2][ibin3]->Draw("LEGO2");
			   ...
			   yield_inc_copy[ibin][ibin2][ibin3] =  (TH2D*) hJetTrackSignalBackgroundLeading[ibin][ibin2][ibin3]->Clone((TString)("Yield_copy"+datalabel+"_"+CBin_strs[ibin]+"_"+CBin_strs[ibin+1]+ "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+"_"+TrkPtBin_strs[ibin3]+"_"+TrkPtBin_strs[ibin3+1]));
				...
				me_projphi[ibin][ibin2][ibin3] =  hJetTrackMELeading[ibin][ibin2][ibin3]->ProjectionY(Form("me_proj_phi%d%d",ibin,ibin2,ibin3),1,100); //what is the 1,100 for?
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(ibin<3){
					...
					pttex->SetNDC();
					pttex->Draw();
					TLatex *Ajtex = new TLatex(0.78,0.85,AjBin_labels[0]);
					Ajtex->SetNDC();
					Ajtex->Draw();
					TLatex *Jettex = new TLatex(0.68, 0.80, JetLabel_labels[0]);
					Jettex->SetNDC();
					Jettex->Draw();
	...
	cc2->SaveAs("cc2_SignalOverME_1To50Phi_EtaProj.png");
