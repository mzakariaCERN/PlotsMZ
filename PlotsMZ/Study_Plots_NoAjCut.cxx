	...
	int Study_Plots_NoAjCut(int i=0)
	{
	...
	//TFile *fin = TFile::Open("Data2011_NoAjCut_Only13_20150505.root");
	//TFile *fin = TFile::Open("Data2011_NoAjCut_Only13_20150508.root");
	//TFile *fin = TFile::Open("Data2011_NoAjCut_Only13_20150511.root");
	TFile *fin_nocut = TFile::Open("Data2011_NoDphiCutForDiJet_Only13_20150511.root");
	TFile *fin = TFile::Open("Data2011_NoDphiCutForDiJet_Only13_20150512.root");
	TFile *fin_PbPbH = TFile::Open("Data_PbPb_Aj_v1_FromH.root");
	//TFile *fin_pp = TFile::Open("Data_pp_trkPtCut1_part0.root");
	TFile *fin_pp = TFile::Open("Data_pp_Aj_v1_FromH.root");
	...
	TCanvas *cc29 = new TCanvas("cc29", "CC29_Aj", 0, 0, 1500, 400);
	TCanvas *cc30 = new TCanvas("cc30", "CC30_PhivsAj", 0, 0, 1500, 1500);
	TCanvas *cc31 = new TCanvas("cc31", "CC31_AjvsdPhi", 0, 0, 1500, 400);
...
if (1) {
	...
	cc29->Divide(4,1,0.0000,0.0000);
if (1) {
    cc29->cd(4);
    gPad->SetRightMargin(0.01);
    gPad->SetTopMargin(0.9);
    cc29->cd(3);
    gPad->SetTopMargin(0.9);
    cc29->cd(2);
    gPad->SetTopMargin(0.9);
...
cc31->Divide(4,1,0.000,0.000);
 ...
 //   gPad->SetRightMargin(0.01);
	...
	TH1D *Aj[nCBins];
	TH1D *Aj_H[nCBins];
	//TH1D *h1[nCBins] = new TH1D("h1%d%d%d","h1%d%d%d",20,0.,1.);	
	//TH1D *h1[nCBins] = new TH1D("h1%d%d%d","h1%d%d%d",20,0.,1.);	
	TH1D *Aj_pp[nCBins];	
	TH1D *dPhi[nCBins];	
	TH2D *Aj_dPhi[nCBins];	
	TH1D *Aj_dPhi_ProjPhi[nCBins];	
	...
	for (int ibin=0;ibin<nCBins;ibin++){
		...
		Aj_H[ibin] = (TH1D*)fin_PbPbH->Get((TString)(desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("AjH_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
		dPhi[ibin] = (TH1D*)fin->Get((TString)(desc + "_dPhi_hist" + CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("dPhi_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
		Aj_dPhi[ibin] = (TH2D*)fin->Get((TString)(desc + "_Aj_dPhi" + CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("Aj_dPhi_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
		Aj_pp[ibin] = (TH1D*)fin_pp->Get((TString)(desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("Aj_pp"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
...
//return 8;
			...
			//	Signal[ibin][ibin2][ibin3]->SetMaximum(MaxSignal[ibin][ibin3]+30);
				...
				Signal[ibin][ibin2][ibin3]->GetYaxis()-> SetLabelSize(0.05);
					...
					if(ibin == 0 && ibin3 == 0)
				Signal[ibin][ibin2][ibin3]->SetMaximum(120);
				if(ibin == 1 && ibin3 == 0)
				Signal[ibin][ibin2][ibin3]->SetMaximum(80);
				if(ibin == 2 && ibin3 == 0)
				Signal[ibin][ibin2][ibin3]->SetMaximum(40);
				if(ibin == 3 && ibin3 == 0)
...
//			cc11->SaveAs("cc11_resultOnPhi_Leading_Under22.png");	
	//##///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
hJetTrackSignalBackgroundLeading[ibin][ibin2][ibin3] = (TH2D*)fin->Get((TString)(desc + "_hJetTrackSignalBackgroundLeading"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+ "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]))->Clone((TString) ("Raw_Yield_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+"_"+TrkPtBin_strs[ibin3]+"_" +TrkPtBin_strs[ibin3+1]));
...

	{
      			...
      			cc18->SaveAs("cc18_MEOverMEO_JetScaled_perEtaperPhi_Leading_Under22.png");
...
hJetTrackSignalBackgroundLeading[ibin][ibin2][ibin3] = (TH2D*)fin->Get((TString)(desc + "_hJetTrackSignalBackgroundLeading"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+ "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]))->Clone((TString) ("Raw_Yield_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+"_"+TrkPtBin_strs[ibin3]+"_" +TrkPtBin_strs[ibin3+1]));
	...
	hJetTrackMELeading[ibin][ibin2][ibin3] = (TH2D*) fin->Get((TString)(desc + "_hJetTrackMELeading"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+ "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]))->Clone((TString) ("Mixed_Event_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + PtBin_strs[ibin2] + "_" + PtBin_strs[ibin2+1]+"_"+TrkPtBin_strs[ibin3]+"_" +TrkPtBin_strs[ibin3+1]));
		...
		me_projeta[ibin][ibin2][ibin3] =  hJetTrackSignalBackgroundLeading[ibin][ibin2][ibin3]->ProjectionX(Form("me_proj_eta%d%d%d",ibin,ibin2,ibin3),18,33); //what is the 1,100 for?
...
//				background[ibin][ibin2][ibin3]->Draw("LEGO2");		
	...
	//			}
				...
				cc24->SaveAs("CC24_S_Scaled_overMEoverM0_NotScaled_PerEtaPerPhi_ProjectionY_etaSideBand.png");	
...
//					Jettex->SetNDC();
	...
	//			}
				...
				cc25->SaveAs("CC25_S_Scaled_overMEoverM0_NotScaled_PerEtaPerPhi_ProjectionY_etaSideBand.png");	
...
//				background_proj[ibin][ibin2][ibin3]->Draw();
		...
		//background[ibin][ibin2][ibin3]->Draw("LEGO2");
			...
			//	Signal[ibin][ibin2][ibin3]->SetMaximum(MaxSignal[ibin][ibin3]+30);
				...
				Signal[ibin][ibin2][ibin3]->GetYaxis()-> SetRangeUser(-0.5, 7);
				Signal[ibin][ibin2][ibin3]->GetYaxis()->SetTitle("#frac{1}{N_{jet}}#frac{d^{2}N}{ d#Delta#phi dp_{T}}");
	...
	//	result[ibin][ibin2][ibin3]->Draw("LEGO2");
				...
				cc26->SaveAs("CC26_S_Scaled_overMEoverM0_NotScaled_PerEtaPerPhi_MinusEtaSideBand_ProjectionY.png");	
...
//				background_proj[ibin][ibin2][ibin3]->Draw();
		...
		//From H	check_old_phi[g][i][j][l] = result2[g][i][j][l]->ProjectionY(summedchecknamePhi,llimiteta,rlimiteta);
			...
			//	check_old_phi_rebin[ibin][ibin2][ibin3]->Draw("Y-", "SAME");
				...
				cc27->SaveAs("CC27_S_ScaledMEoverM0_NotScaled_PerEtaPerPhi_MinusEtaSideBand_ProjectionY.png");
...
//return 27;
				...
				llimiteta = result[ibin][ibin2][ibin3]->GetXaxis()->FindBin(-0.4 + 0.0001); 
         			rlimiteta = result[ibin][ibin2][ibin3]->GetXaxis()->FindBin(0.4 - 0.0001); 
		...
		//		Signal[ibin][ibin2][ibin3] -> Draw();
			...
			//	Signal[ibin][ibin2][ibin3]->SetMaximum(MaxSignal[ibin][ibin3]+30);
				...
				cc28->SaveAs("CC28_SubtractBeforeFilling.png");
	

		cc29->cd((4*(ibin3+1)-ibin));
		//cc29->cd((ibin));
		...
		gPad->SetTicky(1);
		Aj[ibin] =    (TH1D*)fin->Get((TString)(desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("Aj_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
		Aj_pp[ibin] = (TH1D*)fin_pp->Get((TString)(desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("Aj_pp_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));   

//cc29->cd((4*(ibin3+1)-ibin));
 
		Aj[ibin]->SetMarkerStyle(20);
		Aj[ibin]->SetMaximum(4.5);

		Aj_pp[ibin]->SetMarkerStyle(20);
		Aj_pp[ibin]->GetXaxis()->SetRangeUser(0.0, 0.7);
		Aj_pp[ibin]-> SetMaximum(4.5);

		//Aj_pp[ibin]->GetXaxis()->SetRangeUser(0, 0.7);
		Aj[ibin]->SetMarkerColor(kRed);
		Aj_pp[ibin]->SetMarkerColor(kBlue);
		//Aj[ibin]->Rebin(4);
		Aj_pp[ibin]->GetXaxis()->SetTitle("A_{j}");
		Aj_pp[ibin]->GetXaxis()-> SetLabelSize(0.06);
		Aj_pp[ibin]->GetYaxis()-> SetLabelSize(0.06);

		Aj_pp[ibin]->GetXaxis()->SetTitleSize(0.25);
		Aj_pp[ibin]->GetYaxis()->SetTitle("1 / (N_{jet}) dN / dA_{j}");
        
		
		//Aj[ibin]->Scale(EtaBinWidth/4.);
		Aj[ibin]->GetYaxis() -> SetTitleOffset(0.8);
		Aj[ibin]->GetXaxis() -> SetTitleOffset(0.8);
		Aj_pp[ibin]->GetXaxis()-> SetTitleSize(0.06);
		Aj_pp[ibin]->GetYaxis()-> SetTitleSize(0.06);
		Aj[ibin]->GetXaxis()-> SetLabelSize(0.06);
		Aj[ibin]->GetXaxis()-> SetTitleOffset(0.8);
	//	Aj[ibin]->GetXaxis()-> SetTitleSize(0.25);
		Aj[ibin]->GetYaxis()-> SetTitleSize(0.06);
		Aj[ibin]->GetXaxis()-> SetLabelSize(0.06);
		Aj[ibin]->GetYaxis()-> SetLabelSize(0.05);
	//	Aj[ibin]->GetYaxis()-> SetRange(0.0,4.5);

//Double_t newBins[12] = {0.05, 0.1 , 0.15 , 0.2 , 0.25 , 0.3 , 0.35 , 0.4, 0.45, 0.5 , 0.55, 0.6};
//Float_t xbins[21] = {0, 0.05, .1, 0.15, .2, 0.25, .3, 0.35, .4, 0.45, .5, 0.55, 0.6, .65, .7, 0.75, .8, 0.85, .9, 0.95, 1.};
//Float_t xbins[11] = {0,  .1, .2,  .3,   .4, .5,  0.6,  .7,  .8,  .9,  1.};
//  TH1F *EFar = new TH1F("EFar","EFar title",10,xbins);
  //Double_t scale_PbPb = Aj[ibin]->GetXaxis()->GetBinWidth(1)/(Aj[ibin]->Integral());
  Double_t scale_PbPb = 1. / (Aj[ibin]->GetXaxis()->GetBinWidth(1) * Aj[ibin]->Integral());
  Double_t scale_PbPbH = 1. / (Aj_H[ibin]->GetXaxis()->GetBinWidth(1) * Aj_H[ibin]->Integral());
//  Double_t scale_PbPb = 1./Aj[ibin]->Integral();
  //Double_t scale_PbPb = 1./Aj[ibin]->GetEntries();
 //  Aj[ibin]->Rebin(12,"hnew",newBins);
  Aj[ibin]->Scale(scale_PbPb);
//  Aj_H[ibin]->Scale(scale_PbPbH);
//Aj[ibin]->GetXaxis()->Set(11, xbins);
//Aj[ibin]->GetXaxis()->Set(10, 0, 1);
//		Aj[ibin]->GetXaxis()-> SetRangeUser(0.0,1);

  //Aj[ibin]->Draw();

//Double_t scale_pp = Aj_pp[ibin]->GetXaxis()->GetBinWidth(1)/(Aj_pp[ibin]->Integral());
  Double_t scale_pp = 1. / (Aj_pp[ibin]->GetXaxis()->GetBinWidth(1) * Aj_pp[ibin]->Integral());
//Double_t scale_pp = 1/Aj_pp[ibin]->Integral();
//Double_t scale_pp = 1./Aj_pp[ibin]->GetEntries();
  		Aj_pp[ibin]->Scale(scale_pp);
		Aj_pp[ibin]-> SetMaximum(4.0);
		Aj_pp[ibin]->Draw();
		//Aj_pp[ibin]->Draw("same");
		Aj[ibin]->Draw("same");

		
		//Aj_H[ibin]->SetMarkerStyle(2);
		//Aj_H[ibin]->SetMarkerColor(kGreen);
		//Aj_H[ibin]->Draw("same");
...
//		Aj[ibin]->Draw();
	...
	//	Aj_pp[ibin]->Draw("same");
	TLatex *centtex2 = new TLatex(0.65,0.85,CBin_labels[ibin]);
		centtex2->SetNDC();
		centtex2->Draw();	
	  TLegend* leg = new TLegend(0.7, 0.7, .85, .8);

   //leg->SetHeader("The Legend Title");
    // leg->AddEntry("Aj[ibin]","Graph with error bars","lep");
       leg->AddEntry(Aj[ibin],"PbPb","p");
     //  leg->AddEntry(Aj_H[ibin],"PbPb_H","p");
       leg->AddEntry(Aj_pp[ibin],"pp","p");
       //leg->AddEntry(Aj_pp[ibin],"PbPb_H","p");
	 leg->SetBorderSize(0);
	...
	leg->Draw();
		...
		cc29->SaveAs("CC29_Aj.png");
		cc29->SaveAs("CC29_Aj.pdf");

//return 29;
//if ((4*(ibin3+1)-ibin) == 16 || (4*(ibin3+1)-ibin) == 12 || (4*(ibin3+1)-ibin) == 8 || (4*(ibin3+1)-ibin) == 4)
{

	cc31->cd((4*(ibin3+1)-ibin));
	gPad->SetTopMargin(0.02);
    	gPad->SetBottomMargin(0.15);
	...
	Double_t dPhiScale = 1. / (dPhi[ibin]->GetXaxis()->GetBinWidth(1) * Aj[ibin]->GetEntries());
	Aj_dPhi[ibin] = (TH2D*)fin->Get((TString)(desc + "_Aj_dPhi" + CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("Aj_dPhi_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
	Aj[ibin] = (TH1D*)fin->Get((TString)(desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("Aj_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
	
//	
	//Aj_dPhi[ibin]->SetMarkerStyle(20);
	//Aj_dPhi[ibin]->SetMarkerColor(ibin+1);
//	Aj_dPhi[ibin]->GetYaxis()->SetTitle("A_{j}");
//	Aj_dPhi[ibin]->GetXaxis()->SetTitle("#Delta #phi");
	
	//Aj_dPhi[ibin]->Draw("LEGO2");
//	Aj_dPhi_ProjPhi[ibin]->SetMaximum(200);
//	Aj_dPhi[ibin]->SetMarkerStyle(20);
//	Aj_dPhi[ibin]->SetMarkerColor(ibin+1);
	Aj_dPhi_ProjPhi[ibin] = Aj_dPhi[ibin]->ProjectionX(Form("ProPhi5%d%d%d", ibin),  0,  5);
	Double_t dPhiScaleVsAj = 1. / (Aj_dPhi_ProjPhi[ibin]->GetXaxis()->GetBinWidth(1) * Aj[ibin]->GetEntries());

//	 Aj_dPhi[ibin]->ProjectionX("ProPhi5%d",  0,  5)->Draw();
	Aj_dPhi_ProjPhi[ibin]->Scale(1./ 5.);
	Aj_dPhi_ProjPhi[ibin]->Scale(dPhiScaleVsAj);

	Aj_dPhi_ProjPhi[ibin]->SetMarkerStyle(20);
	Aj_dPhi_ProjPhi[ibin]->SetMarkerColor(kBlack);
	Aj_dPhi_ProjPhi[ibin]->SetLineColor(kBlack);
	Aj_dPhi_ProjPhi[ibin]-> SetMaximum(1.0);
	Aj_dPhi_ProjPhi[ibin]-> SetMinimum(0.009);

	Aj_dPhi_ProjPhi[ibin]->GetYaxis() -> SetTitleOffset(0.8);
	Aj_dPhi_ProjPhi[ibin]->GetXaxis() -> SetTitleOffset(1.0);
	Aj_dPhi_ProjPhi[ibin]->GetXaxis()-> SetLabelSize(0.06);
	Aj_dPhi_ProjPhi[ibin]->GetYaxis()-> SetLabelSize(0.06);
	Aj_dPhi_ProjPhi[ibin]->GetXaxis()->SetTitle("|#Delta#phi|");
	Aj_dPhi_ProjPhi[ibin]->GetYaxis()->SetTitle("1 / N_{jet} dN / d(#Delta#phi)");
	Aj_dPhi_ProjPhi[ibin]->GetXaxis()->SetTitleSize(0.06);

	Aj_dPhi_ProjPhi[ibin]->Draw();

	TLegend* leg2 = new TLegend(0.20, 0.6, .35, .8);
	leg2->AddEntry(Aj_dPhi_ProjPhi[ibin],"0 < A_{j} < 0.15","p");
	leg2->SetBorderSize(0);
  	leg2->SetTextSize(0.06);
	leg2->Draw();


//	Aj_dPhi_ProjPhi[ibin] = Aj_dPhi[ibin]->ProjectionX("ProPhi15%d%d%d",  6, 15);
	Aj_dPhi_ProjPhi[ibin] = Aj_dPhi[ibin]->ProjectionX(Form("ProPhi15%d%d%d", ibin),  6, 15);
	 dPhiScaleVsAj = 1. / (Aj_dPhi_ProjPhi[ibin]->GetXaxis()->GetBinWidth(1) * Aj[ibin]->GetEntries());
	Aj_dPhi_ProjPhi[ibin]->Scale( dPhiScaleVsAj);
	
//	 Aj_dPhi[ibin]->ProjectionX("ProPhi15",  6, 15)->Draw("same");
	Aj_dPhi_ProjPhi[ibin]->Scale(1./ 9.);
	Aj_dPhi_ProjPhi[ibin]->SetMarkerStyle(20);
	Aj_dPhi_ProjPhi[ibin]->SetMarkerColor(kBlue);
	Aj_dPhi_ProjPhi[ibin]->SetLineColor(kBlue);
	Aj_dPhi_ProjPhi[ibin]->Draw("same");

   leg2->AddEntry(Aj_dPhi_ProjPhi[ibin],"0.15 < A_{j} < 0.45","p");
		 leg2->SetBorderSize(0);
  	leg2->SetTextSize(0.06);
	leg2->Draw();


//	Aj_dPhi_ProjPhi[ibin] = Aj_dPhi[ibin]->ProjectionX("ProPhi25%d%d%d",  16, 25);
	Aj_dPhi_ProjPhi[ibin] = Aj_dPhi[ibin]->ProjectionX(Form("ProPhi25%d%d%d", ibin),  16, 25);
//	 Aj_dPhi[ibin]->ProjectionX("ProPhi25",  16, 25)->Draw("same");
	 dPhiScaleVsAj = 1. / (Aj_dPhi_ProjPhi[ibin]->GetXaxis()->GetBinWidth(1) * Aj[ibin]->GetEntries());
	Aj_dPhi_ProjPhi[ibin]->Scale( dPhiScaleVsAj);

	Aj_dPhi_ProjPhi[ibin]->Scale(1./ 9.);
	Aj_dPhi_ProjPhi[ibin]->SetMarkerStyle(20);
	Aj_dPhi_ProjPhi[ibin]->SetMarkerColor(kRed);
	Aj_dPhi_ProjPhi[ibin]->SetLineColor(kRed);

	Aj_dPhi_ProjPhi[ibin]->Draw("same");

	  leg2->AddEntry(Aj_dPhi_ProjPhi[ibin],"0.45 < A_{j} < 0.75","p");
	 leg2->SetBorderSize(0);
  	leg2->SetTextSize(0.06);
	leg2->Draw();
   ...
   //leg->SetHeader("The Legend Title");
    // leg->AddEntry("Aj[ibin]","Graph with error bars","lep");
         //  leg->AddEntry(Aj_H[ibin],"PbPb_H","p");
    //   leg2->AddEntry(Aj_pp[ibin],"pp","p");
       //leg->AddEntry(Aj_pp[ibin],"PbPb_H","p");

//cout << "the vlaue of ibin = " << ibin << endl;	
//	
//	centtex2->Draw();	
//
	TLatex *centtex31 = new TLatex(0.15,0.85,CBin_labels[ibin]);
	centtex31->SetNDC();
	centtex31->Draw();

	cc31->SaveAs("CC31_AjvsdPhi.png");
	cc31->SaveAs("CC31_AjvsdPhi.pdf");
}		
	 ...
	 cc30->SetLogy();	
		dPhi[ibin] =    (TH1D*)fin_nocut->Get((TString)(desc + "_dPhi_hist" + CBin_strs[ibin] + "_" + CBin_strs[ibin+1]))->Clone((TString) ("dPhi_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]));    
        	Double_t dPhiScale = 1. / (dPhi[ibin]->GetXaxis()->GetBinWidth(1) * Aj[ibin]->GetEntries());
        	//Double_t dPhiScale = 1. / ( Aj[ibin]->GetEntries());
        	//Double_t dPhiScale = 1. / (dPhi[ibin]->GetXaxis()->GetBinWidth(1));
		gPad->SetTickx(1);
		gPad->SetTicky(1);
	
		dPhi[ibin]->Scale(dPhiScale);
		dPhi[ibin]->GetXaxis()->SetTitle("|#Delta#phi|");
		dPhi[ibin]->GetYaxis()->SetTitle("1 / N_{jet} dN / d(#Delta#phi)");

		dPhi[ibin]->SetMarkerStyle(20);
		dPhi[ibin]->SetMarkerSize(1.5);
		dPhi[ibin]->SetMarkerColor(ibin+1);
		dPhi[ibin]->SetLineColor(ibin+1);
        //	dPhi[ibin]->GetYaxis()-> SetRangeUser(1.0,100000);
        	dPhi[ibin]->GetYaxis()-> SetRangeUser(0.0001, 1.0);
...
//		if (i == 1)
		dPhi[ibin]->Draw("same");
			  ...
			  TLegend* leg2 = new TLegend(0.2, 0.65, .45, .85);

   //leg->SetHeader("The Legend Title");
    // leg->AddEntry("Aj[ibin]","Graph with error bars","lep");
       leg2->AddEntry(dPhi[0],"Cent. 0-%10","p");
       leg2->AddEntry(dPhi[1],"Cent. %10-%30","p");
       leg2->AddEntry(dPhi[2],"Cent. %30-%50" ,"p");
       leg2->AddEntry(dPhi[3],"Cent. %50-%100","p");
       //leg->AddEntry(Aj_pp[ibin],"pp","p");
	 leg2->SetBorderSize(0);
  leg2->Draw();

		
		cc30->SaveAs("CC30_dPhi.png");
		cc30->SaveAs("CC30_dPhi.pdf");
