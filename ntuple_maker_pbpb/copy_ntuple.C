...
//	  TString output_file_base = "root_output/FullOfficialReco_PbPbData_mini_ntuple_v1";
	  ...
	  output_file->cd();
	  int ev_min = output_file_num*70000; // int ev_min = output_file_num*100000;
	  int ev_max = ev_min + 69999; //	  int ev_max = ev_min + 99999; 
	  ...
	  for(int evi = ev_min; evi < ev_max; evi++) {
	    ...
	    my_ct->fChain->GetEntry(evi);	    //  my_ct2->fChain->GetEntry(evi);
  ...
  ///-----------------------------------------------------------------------------
	    ...
	    for(int j4i = 0; j4i < my_ct->nref ; j4i++) {
	// MZ same step done at the histogram       if( fabs(my_ct->jteta[j4i]) > 2 ) continue;		
...
// MZ I think similar selections were made atthe plots  if(my_ct5->highPurity[itrk]==0 || my_ct5->trkPtError[itrk]/my_ct5->trkPt[itrk]>=0.1 || TMath::Abs(my_ct5->trkDz1[itrk]/my_ct5->trkDzError1[itrk])>=3.0 ||TMath::Abs(my_ct5->trkDxy1[itrk]/my_ct5->trkDxyError1[itrk])>=3.0) continue ;
