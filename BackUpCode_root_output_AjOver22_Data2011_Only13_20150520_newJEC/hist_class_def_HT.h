...
class hist_class {
 public:
  ...
  TH1F* dPhiNoCut_hist[nCBins]; //MZ no cut on dijets
  TH1F* Aj[nCBins];
  TH2F* Aj_dPhi[nCBins];
...
hist_class::hist_class(TString the_desc, bool is_it_data) {
  ...
  NEvents = new TH1F((TString) (desc + "_Nevents"), "This saves the centrality (btw 0 and 100)", 100, 0., 100.);     NEvents->Sumw2(); 
  ...
  for (int ibin=0;ibin<nCBins;ibin++){
    ...
    dPhiNoCut_hist[ibin] = new TH1F((TString) (desc + "_dPhiNoCut_hist"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]), "", 30, 0.,TMath::Pi());     dPhiNoCut_hist[ibin]->Sumw2();
    Aj[ibin] = new TH1F((TString) (desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]), "", 25,0., 0.75);     Aj[ibin]->Sumw2();
    //Aj[ibin] = new TH1F((TString) (desc + "_Aj"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1]), "", 10,0.,1.0);     Aj[ibin]->Sumw2(); //MZ 20150507
...
void hist_class::AddHists(hist_class *more_hists, float wt)
{
  ...
  for (int ibin=0;ibin<nCBins;ibin++){
    ...
    dPhiNoCut_hist[ibin]->Add(more_hists->dPhiNoCut_hist[ibin],wt);
